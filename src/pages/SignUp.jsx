...
import { FarmerRegistrationFields } from "@/components/forms/FarmerRegistrationFields";
import { AgentRegistrationFields } from "@/components/forms/AgentRegistrationFields";
...
export default function SignUp() {
  ...
  const validate = () => {
    ...
    if (isFarmerRole) {
      if (!form.nationalIdImage) {
        e.nationalIdImage = "National ID image is required for farmers.";
      }
      if (!form.farmRegistrationImage) {
        e.farmRegistrationImage =
          "Farm registration image is required for farmers.";
      }
      if (!form.businessCertificateImage) {
        e.businessCertificateImage =
          "Business certificate image is required for farmers.";
      }
      if (!form.assignedRegion) {
        e.assignedRegion = "Region is required for farmers.";
      }
    }
    if (isAgentRole) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isFarmerRole) {
        payload.append("region", form.assignedRegion);
        payload.append("nationalId", form.nationalIdImage);
        payload.append("farmRegistration", form.farmRegistrationImage);
        payload.append("businessCertificate", form.businessCertificateImage);
      } else if (isAgentRole) {
        payload.append("assignedRegion", form.assignedRegion);
        payload.append("commissionRate", form.commissionRate);
          ...
          {success ? (
            ...
            >
              ...
              <form onSubmit={handleSubmit} noValidate>
                ...
                <AnimatePresence>
                  {isFarmerRole && (
                    <FarmerRegistrationFields form={form} setForm={setForm} errors={errors} setErrors={setErrors} />
                  )}

                  {isAgentRole && (
                    <AgentRegistrationFields form={form} setForm={setForm} errors={errors} setErrors={setErrors} regions={GHANA_REGIONS} />
                  )}
                </AnimatePresence>
